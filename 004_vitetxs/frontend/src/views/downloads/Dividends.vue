<template>
  <div class="download-card">
    <h3>Dividends</h3>
    <div class="content">
      <p class="description">Download statistics about staked VX on ViteX by address as .csv file. It includes only received dividends.</p>
      ...
      <download-button :onSubmit="getDataFromApi" :requestInProgress="this.$store.state.dividends.loading"/>
    </div>
    <v-snackbar
      v-model="snackbar"
      :timeout="7500"
...
export default {
  ...
  methods: {
    async getDataFromApi() {
      await this.$store.dispatch("dividends/getStatistics", this.viteAddress);
      this.snackbar = this.$store.state.dividends.error;
    }
  },
  watch: {
